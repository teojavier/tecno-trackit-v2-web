...
@section('content')
    <div class="px-3 py-5">
        <div class="text-start flex">
            <h2 class="text-lg font-bold text-primary-dark dark:text-light">Articulos / Editar ({{ $contador }})</h2>
        </div>
    </div>
            ...
            <div class="">
                ...
                <form action="{{ route('articles.update', $articulo->id) }}" method="post">
                    ...
                    <div class="mt-3">
                        ...
                        <textarea name="content" id="summary-ckeditor" cols="30" rows="10">{{ $articulo->content }}</textarea>
                        @error('content')
                            <span class="text-left text-red-500 ">Inserte Datos Correctos</span>
                        @enderror
                    </div>
